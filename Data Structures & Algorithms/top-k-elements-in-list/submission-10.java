...
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        ...
        // Bucket sort uses index to count frequency. instead of maintaining frequency we usae index.
        // Max frequency can be equal to size of given array but since array index starts with 0 create bucket size equal to nums.length+1. 
        ...
        /*
           The issue with the iteration range 'i >= bucket.length - k' is that it might not cover all 
           the necessary buckets if the most frequent elements are spread across more than k buckets.
           So the correct iteration range is 'i >= 0 && j < k'
        */
        for (int i = bucket.length - 1; i >= 0 && j < k; i--) {
           if(!bucket[i].isEmpty()) {
              for (int num : bucket[i]) {
                result[j++] = num;
                if (j == k) break; // answer is aways unique, so dont need this line though
             }
           }
        }
