/* Algo:
This is a classic problem of dynamic increasing the window size as well as dynamic decreasing
the window size.
   1. Hash the entire t
   2. Use variable size sliding window to Iterate over s and keep on hashing s one by one
   3. When valid window is found, then keep contracting the window until you find smallest window
      (Keep variables to track smallest window length and string)
   4. Wile contracting decrement the hashed characters of s which is no longer valid for window
*/
class Solution {
    public String minWindow(String s, String t) {
        ...
        /* 
           If found atleast one matching character, start the window
...
/**
Use the hashed t and match the characters in s. A window is valid if s has greater or equal characters than t
**/
